...
app.put("/repositories/:id", (request, response) => {
  ...
  const findRepositoryById = repositories.findIndex(
    (repository) => repository.id === id
  );

  if (findRepositoryById === -1) {
...
app.post("/repositories/:id/like", (request, response) => {
  ...
  const findRepositoryById = repositories.find(
    (repository) => repository.id === id
  );

  if (!findRepositoryById) {
    return response.status(400).json({ error: "Repositorio não existe." });
  }

  findRepositoryById.likes++;

  return response.json(findRepositoryById);
});
